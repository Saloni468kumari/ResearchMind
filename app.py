...
# ── Custom Responsive Dark UI CSS (Your Original Style) ───────────────────────
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600;700&family=Inter:wght@300;400;500;600&display=swap');
...
with st.sidebar:
    ...
    <div style='display:flex;align-items:center;gap:10px;margin-bottom:20px'>
      ...
      <div style='min-width:0'>
        <div style='font-family:"JetBrains Mono",monospace;font-weight:700;
                    font-size:14px;color:#F0F0F0;white-space:nowrap;
                    overflow:hidden;text-overflow:ellipsis'>ResearchMind</div>
        <div style='font-size:10px;color:#555555;margin-top:1px'>AI Pipeline · 4 Stages</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("<hr style='margin:16px 0'>", unsafe_allow_html=True)
    st.markdown('<div class="mono-label" style="margin-bottom:10px">Agent Pipeline</div>', unsafe_allow_html=True)

    # Render Sidebar Progress Tracker Cards
    for i, (sid, icon, label, desc, color) in enumerate(STAGES):
        status = stage_status(sid)
        border = color + "80" if status == "done" else (color + "50" if status == "running" else "#3A3A3A")
        bg = f"linear-gradient(135deg,{color}18,#242424)" if status in ("done","running") else "#2E2E2E"
    ...
    # Live log box in sidebar
    st.markdown('<div class="mono-label" style="margin-bottom:6px">Live Log</div>', unsafe_allow_html=True)
    log_text = "\n".join(st.session_state.log[-15:]) if st.session_state.log else "Pipeline Idle"
    st.code(log_text, language=None)
...
# Top row interactive metric status indicators
col1, col2, col3, col4 = st.columns(4)
for col, (sid, icon, label, _, color) in zip([col1, col2, col3, col4], STAGES):
    s = stage_status(sid)
    badge = "✓ Done" if s=="done" else ("⏳ Running" if s=="running" else "— Idle")
    badge_col = "#4ADE80" if s=="done" else (color if s=="running" else "#555555")
    border = color+"60" if s!="idle" else "#2E2E2E"
    glow = f"0 0 16px {color}25" if s=="running" else "none"
    with col:
        st.markdown(f"""
        <div class="metric-card" style='border:1.5px solid {border};box-shadow:{glow}'>
          <div class="metric-icon">{icon}</div>
          <div class="metric-label" style='color:{color if s!="idle" else "#666666"}'>{label}</div>
          <div class="metric-badge" style='color:{badge_col}'>{badge}</div>
        </div>
        """, unsafe_allow_html=True)
...
def render_output(content, placeholder, color):
    if content:
        st.markdown(f"""<div class="output-box" style='border-left:3px solid {color}'>{content}</div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div style='background:#242424;border:1px solid #2E2E2E;border-radius:10px;
                    padding:50px 20px;text-align:center;min-height:200px'>
          <div style='font-size:32px;opacity:0.15'>⏳</div>
          <div style='font-family:"JetBrains Mono",monospace;font-size:12px;
                      color:#444444;margin-top:10px'>{placeholder}</div>
        </div>
        """, unsafe_allow_html=True)
...
if run_clicked:
    if not topic.strip():
        st.sidebar.warning("Please enter a research topic in the sidebar.")
    elif not PIPELINE_AVAILABLE:
        st.sidebar.error("⚠️ `agents.py` not found or configurations missing.")
    else:
        st.session_state.update({
            "search_results": "", "scraped_content": "",
            "report": "", "feedback": "",
        ...
        add_log("Pipeline Init — Activating Agents...")
        st.rerun()

# ── BUG-FREE STATE EXECUTION ENGINE ───────────────────────────────────────────
if st.session_state.running:
    try:
        ...
        if stage == "search":
            with st.spinner("🔍 Search Agent scanning the web..."):
                agent = build_search_agent()
                result = agent.invoke({"messages": [("user", f"Find recent, reliable and detailed information about: {topic_val}")]})
                st.session_state.search_results = result["messages"][-1].content
                add_log("Search complete ✓")
                st.session_state.stage = "reader"
                st.rerun()
            ...
            with st.spinner("📄 Reader Agent scraping top resources..."):
                agent = build_reader_agent()
                result = agent.invoke({"messages": [("user", 
                    f"Based on the following search results about '{topic_val}', pick the most relevant URL and scrape it for deeper content.\n\nSearch Results:\n{st.session_state.search_results[:800]}"
                )]})
                st.session_state.scraped_content = result["messages"][-1].content
                add_log("Scraping complete ✓")
                st.session_state.stage = "writer"
            ...
            with st.spinner("✍️ Writer Chain drafting the report..."):
                ...
                st.session_state.report = writer_chain.invoke({"topic": topic_val, "research": combined})
                add_log("Report drafted ✓")
                st.session_state.stage = "critic"
                st.rerun()
        ...
        st.session_state.error_msg = str(e)
        st.session_state.error_at = st.session_state.stage
        st.session_state.stage = "error"
        st.session_state.running = False
        add_log(f"❌ Error: {e}")
        st.rerun()